...
result_a = annotate_spectral_centroids(ax_a, image_a, 9.01, ...
    wavelength_calibration, nir_range_um, mir_range_um, mir_energy_mJ, true, fs);
...
result_c = annotate_spectral_centroids(ax_c, image_c, 9.7, ...
    wavelength_calibration, nir_range_um, mir_range_um, mir_energy_mJ, false, fs);
...
function result = annotate_spectral_centroids(ax, spectrum, position_800nm, ...
    ...
    result.nir_energy_mJ = NaN;
    if show_mir && result.mir.integral > 0
        result.nir_energy_mJ = mir_energy_mJ * ...
            result.nir.integral / result.mir.integral;
    end
    ...
    if show_mir && result.mir.integral > 0
        ...
        text(ax, result.mir.x, result.mir.y, ...
            ...
            'Clipping', 'on');
        text(ax, 0.97, 0.05, ...
            sprintf('Estimated NIR energy: %.0f mJ', result.nir_energy_mJ), ...
...
function print_spectral_result(label, result)
    ...
    if isfinite(result.nir_energy_mJ)
        fprintf('  Estimated NIR energy from area ratio (MIR = 31 mJ): %.3f mJ\n', ...
            result.nir_energy_mJ);
    else
        fprintf('  NIR energy estimate: not evaluated (no reliable MIR reference).\n');
    end
end
